...
- 冲线后先播放最后赛段结算，再按实际入堆顺序逐张翻开冠军与垫底暗注，显示命中/失败及 `+8/+5/+3/+2/+1/−1`，最后显示支持并列冠军的金币排名。
...
- `node --test test/camelup-official.test.js`：9/9 通过；最终全量回归 359/359 通过。该结果代表本轮自动化与浏览器验收全部通过，不扩大表述为所有未覆盖的官方规则情形都已经 100% 人工验收。
