...
class JungleEngine {
    ...
    handleStudySetup(playerId, action = {}) {
        ...
        const resetMeta = message => { this.status = 'playing'; this.winner = null; this.lastMove = null; this.lastAction = null; this.actionLog = [message]; return this._success(message); };
        if (action.kind === 'reset') { this.board = this._createBoard(); this.turn = 'red'; this.currentTurnIndex = 0; return resetMeta('已恢复斗兽棋标准开局'); }
        if (action.kind === 'clear') { this.board = new Map(); this.turn = 'red'; this.currentTurnIndex = 0; return resetMeta('已清空斗兽棋局面'); }
        if (action.kind === 'setTurn') { if (!['red', 'blue'].includes(action.color)) return { success: false, message: '行动方无效', state: this.getPlayerState(playerId) }; this.turn = action.color; this.currentTurnIndex = action.color === 'red' ? 0 : 1; this.status = 'playing'; this.winner = null; return this._success(`下一手为${action.color === 'red' ? '红方' : '蓝方'}`); }
        if (action.kind === 'remove') { const point = coord({ x: action.x, y: action.y }); if (!point || !this.board.delete(key(point.x, point.y))) return { success: false, message: '该位置没有可移除的兽子', state: this.getPlayerState(playerId) }; return resetMeta('已移除兽子'); }
        if (action.kind === 'place') { const point = coord({ x: action.x, y: action.y }); const color = action.color === 'blue' ? 'blue' : action.color === 'red' ? 'red' : player.color; const type = PIECES[action.pieceType || action.type] ? (action.pieceType || action.type) : 'r'; if (!point || this.board.has(key(point.x, point.y))) return { success: false, message: '摆棋位置无效或已有兽子', state: this.getPlayerState(playerId) }; this.board.set(key(point.x, point.y), { id: `study-${color}-${Date.now()}-${this.board.size}`, x: point.x, y: point.y, type, color }); return resetMeta('已摆放兽子'); }
        if (action.kind === 'move') { const from = coord(action.from); const to = coord(action.to); const piece = from && this.board.get(key(from.x, from.y)); if (!from || !to || !piece || piece.color !== player.color || this.board.has(key(to.x, to.y))) return { success: false, message: '只能移动当前执棋方的兽子到空位', state: this.getPlayerState(playerId) }; this.board.delete(key(from.x, from.y)); this.board.set(key(to.x, to.y), { ...piece, x: to.x, y: to.y }); return resetMeta('已调整兽子位置'); }
