...
test('Werewolf WebSocket lifecycle wires transient disconnect and resume hooks', () => {
    ...
    assert.match(appSource, /handlePlayerDisconnect\?\.\(player\.id\)/);
...
test('Werewolf lobby requires host room configuration before listing and starting', () => {
    ...
    assert.match(room, /this\.players\.length !== this\.targetPlayers/);
...
test('Witch Town completes a six-player game through night protection and final trial reveals', () => {
    ...
    game.players.forEach(player => { player.trialCards = Array.from({ length: 5 }, (_, index) => trial('town', `${player.id}-town-${index}`)); player.everWitch = false; player.everConstable = false; player.identity = 'villager'; player.eliminated = false; });
