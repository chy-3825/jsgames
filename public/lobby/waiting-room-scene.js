...
export function createWaitingRoomScene({
    ...
    getStartGameRequestPending = () => false,
    ...
    // A seat that just left remains as a keyed visual tombstone until its
    // shared fade deadline.  Keeping this outside the HTML string prevents a
    // room snapshot/reconnect repaint from cutting the fade short.
    const waitingSeatExtinguishVisuals = new Map();
    ...
    const clearVisualTimers = () => {
        ...
        waitingSeatExtinguishVisuals.clear();
    ...
    function getWaitingRoomState() {
        ...
        const playerProgress = fixedPlayerCount
            ? (connectedPlayers.length === targetPlayers ? '玩家已到齐' : `还需 ${Math.max(0, targetPlayers - connectedPlayers.length)} 名玩家加入`)
            : (connectedPlayers.length >= minPlayers ? `已满足人数要求，剩余 ${Math.max(0, targetPlayers - connectedPlayers.length)} 个空位` : `至少还需 ${Math.max(0, minPlayers - connectedPlayers.length)} 名玩家加入`);
        const preload = getGamePreloadState(currentRoom.gameType);
        const playerCountReady = currentRoom.targetPlayers ? connectedPlayers.length === targetPlayers : connectedPlayers.length >= minPlayers;
        return {
            players,
            minPlayers,
            isHost,
            ...
            allPlayersReady,
            playerCountReady,
            startGameRequestPending: Boolean(getStartGameRequestPending()),
            canStart: isHost && !getStartGameRequestPending() && !needsConfiguration && playerCountReady && allPlayersReady && preload.status === 'ready',
        };
    }
    ...
    function waitingRoomStartPresentation(waiting) {
        const count = waiting.connectedPlayers.length;
        ...
        let state = 'locked';
        let label = '未达到开始游戏所需人数';
        let detail = missing > 0 ? `还需 ${missing} 名玩家加入` : '';
        if (waiting.startGameRequestPending) {
            state = 'starting';
            label = '正在开始游戏';
            detail = '正在同步房间状态';
        } else if (waiting.needsConfiguration) {
            state = 'setup';
            label = '请先完成房间设置';
    ...
    function waitingRoomMagicMarkup(waiting) {
        ...
        const canTrigger = interactive && state === 'ready' && !waiting.startGameRequestPending;
        const trigger = interactive
            ? `<button class="pregame-magic-trigger" data-start-game data-start-state="${state}" type="button" ${canTrigger ? '' : 'disabled'} aria-disabled="${canTrigger ? 'false' : 'true'}" aria-label="${escapeHtml(label)}，${escapeHtml(detail)}">${circle}</button>`
    ...
    function waitingRoomStartGuidanceMarkup(waiting) {
        ...
        const requiredPlayers = waiting.fixedPlayerCount ? waiting.targetPlayers : waiting.minPlayers;
        const playerCountReady = waiting.connectedPlayers.length >= requiredPlayers && !waiting.needsConfiguration && waiting.allPlayersReady;
    ...
    function renderWaitingRoomScene() {
        ...
        const seats = Array.from({ length: capacity }, (_, seatIndex) => {
            ...
            const existingExtinguish = waitingSeatExtinguishVisuals.get(seatIndex);
            if (player && existingExtinguish && existingExtinguish.player.id !== player.id) {
                windowRef.clearTimeout(waitingSeatExtinguishTimers.get(seatIndex));
                waitingSeatExtinguishTimers.delete(seatIndex);
                waitingSeatExtinguishVisuals.delete(seatIndex);
            } else if (player && existingExtinguish) {
                windowRef.clearTimeout(waitingSeatExtinguishTimers.get(seatIndex));
                waitingSeatExtinguishTimers.delete(seatIndex);
                waitingSeatExtinguishVisuals.delete(seatIndex);
            }
            let leavingVisual = !player ? waitingSeatExtinguishVisuals.get(seatIndex) : null;
            if (!player && previousPlayer && (!leavingVisual || leavingVisual.player.id !== previousPlayer.id)) {
                if (windowRef.matchMedia?.('(prefers-reduced-motion: reduce)').matches) {
                    waitingSeatExtinguishVisuals.delete(seatIndex);
                    leavingVisual = null;
                } else {
                    leavingVisual = { player: previousPlayer, startedAt: Date.now(), until: Date.now() + 720 };
                    waitingSeatExtinguishVisuals.set(seatIndex, leavingVisual);
                }
            }
            const leaving = Boolean(!player && leavingVisual);
            const visualPlayer = player || leavingVisual?.player || null;
            ...
            if (player && player.id === previous?.player?.id && player.ready !== true && previous.player.ready === true) {
                waitingSeatIgniteUntil.delete(seatIndex);
                windowRef.clearTimeout(waitingSeatIgniteTimers.get(seatIndex));
                waitingSeatIgniteTimers.delete(seatIndex);
            }
            ...
            const igniteStartedAt = (waitingSeatIgniteUntil.get(seatIndex) || 0) - 840;
            const igniteElapsed = isIgniting ? Math.max(0, Date.now() - igniteStartedAt) : 0;
            const extinguishElapsed = isExtinguishing ? Math.max(0, Date.now() - (leavingVisual?.startedAt || Date.now())) : 0;
            const seatClasses = [
            ...
            const seatStyle = `${waitingSeatStyle(seatIndex, capacity, mySeatIndex)};--ignite-elapsed:${igniteElapsed}ms;--extinguish-elapsed:${extinguishElapsed}ms`;
            return `<article class="${seatClasses}" style="${seatStyle}" data-seat-index="${seatIndex}" aria-label="${seatAriaLabel}"><span class="pregame-chair" aria-hidden="true"></span><span class="pregame-seat-fire" aria-hidden="true"><i class="pregame-fire-glow"></i><i class="pregame-fire-flame"></i><i class="pregame-fire-core"></i></span><span class="pregame-seat-copy"><strong>${seatName}</strong><small>${seatLabel}${readyLabel ? ` · ${readyLabel}` : ''}</small></span><b>${String(seatIndex + 1).padStart(2, '0')}</b></article>`;
        }).join('');
        waitingSeatVisualSnapshot = nextSeats;
        const preloadLabel = waiting.preload.status === 'ready' ? '游戏资源已加载' : waiting.preload.status === 'error' ? '游戏资源加载失败' : '正在加载游戏资源';
        ...
        const preloadDetail = waiting.preload.status === 'error' && waiting.preload.error ? `<small>${escapeHtml(waiting.preload.error)}</small>` : '';
        mount.innerHTML = `<section class="pregame-room tone-${meta.tone || 'default'} ${presetLayout ? 'is-preset-layout' : ''}" data-game-type="${escapeHtml(currentRoom.gameType)}" style="--pregame-art:url('${escapeHtml(art)}')"><div class="pregame-backdrop" aria-hidden="true"></div><main class="pregame-stage" data-seat-count="${capacity}"><div class="pregame-table" aria-label="${escapeHtml(meta.title || currentRoom.gameName)}等待牌桌"><div class="pregame-table-action">${tableAction}</div></div><div class="pregame-seats">${seats}</div></main>${startGuidance}<footer class="pregame-room-footer"><div class="pregame-resource-state">${preloadAction}<span>${preloadLabel}${preloadDetail}</span></div><div><span>${waiting.connectedPlayers.length} / ${waiting.targetPlayers} 已入座</span></div></footer>${roomSettingsMarkup}</section>`;
    }
