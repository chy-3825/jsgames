...
export function renderInfluence(card, options = {}) {
    ...
    const attrs = Number.isInteger(options.lossIndex)
        ...
        : Number.isInteger(options.exchangeIndex)
            ...
            : '';
    const faceMarkup = face ? `<img class="cp-card-art" src="/assets/games/coup/art-deco-v2/${ROLE_ART[role]}.jpg" alt="${escapeHtml(`${ROLE_NAMES[role]}牌面`)}">${!options.mini ? `<span class="cp-card-copy"><i>${ROLE_MARKS[role]}</i><strong>${ROLE_NAMES[role]}</strong><small>${ROLE_EFFECTS[role]}</small></span>` : ''}` : `<span class="cp-card-back-art" aria-hidden="true"><img src="/assets/games/coup/art-deco-v2/${CARD_BACK_ART}.jpg" alt=""></span>`;
...
export function renderSceneRole(role) {
    ...
    return `<div class="cp-scene-role is-role-${role}"><img src="/assets/games/coup/art-deco-v2/${ROLE_ART[role]}.jpg" alt="${escapeHtml(ROLE_NAMES[role])}牌面"><strong>${escapeHtml(ROLE_NAMES[role])}</strong></div>`;
}

export function renderBlockDeclaration(role) {
    if (role === 'captain_or_ambassador') {
        return `<span class="cp-block-card is-dual"><span><img src="/assets/games/coup/art-deco-v2/${ROLE_ART.captain}.jpg" alt="船长"><img src="/assets/games/coup/art-deco-v2/${ROLE_ART.ambassador}.jpg" alt="大使"></span><b>阻挡声明</b></span>`;
    }
    if (!ROLE_ART[role]) return '';
    return `<span class="cp-block-card is-role-${role}" data-role-card="${role}"><img src="/assets/games/coup/art-deco-v2/${ROLE_ART[role]}.jpg" alt="${escapeHtml(ROLE_NAMES[role])}阻挡声明"><b>阻挡声明</b></span>`;
}

export function renderActionVisual(action) {
    if (action?.role && ROLE_ART[action.role]) return `<span class="cp-action-portrait is-role-${action.role}"><img src="/assets/games/coup/art-deco-v2/${ROLE_ART[action.role]}.jpg" alt=""></span>`;
