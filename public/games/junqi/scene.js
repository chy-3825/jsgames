...
import { BOARD_DETAIL_Y, BOARD_LINE_Y, BOARD_TOP, CAMPS, CAMP_RADIUS_X, CAMP_RADIUS_Z, CAMP_SVG_RX, CAMP_SVG_RY, DEFAULT_CAMERA_DISTANCE, DEFAULT_CAMERA_PITCH, HEADQUARTERS, HEIGHT, HQ_HALF_X, HQ_HALF_Z, HQ_SVG_HALF_X, HQ_SVG_HALF_Y, LABELS, PIECE_BASE_Y, PIECE_BODY_DEPTH, PIECE_BODY_HEIGHT, PIECE_BODY_WIDTH, PIECE_BODY_Y, RAIL_ROWS, SHORT_LABELS, STATION_HALF_X, STATION_HALF_Z, WIDTH, WORLD_X, WORLD_Z } from './constants.js';
...
export function createJunqiScene({ mount, model, send, addLog }) {
    ...
    try { renderer = new THREE.WebGLRenderer({ canvas, antialias: true, powerPreference: 'high-performance' }); } catch (error) {
        ...
        return { gameType: 'junqi', handleMessage() {}, getActionBindings() { return null; }, destroy() {} };
    }
    ...
    function renderPlayer(element, player, color) {
        ...
        element.innerHTML = `<div class="junqi3d-player-heading"><span class="junqi3d-player-color">${color === 'red' ? '红方' : '蓝方'}</span><small>${player.id === state.myId ? '我方' : '对手'}</small></div><div class="junqi3d-player-main"><div><strong>${escapeHtml(player.name)}</strong><small>${status}</small></div></div><div class="junqi3d-strength"><span><b>${alive}</b> 在场</span><span><b>${25 - alive}</b> 损失</span></div>`;
    }
    ...
    return {
        ...
        destroy() {
            ...
            pieceBodyMaterials.forEach(material => material.dispose());
            pieceFaceMaterials.forEach(material => material.dispose());
            Object.values(materials).forEach(material => material.dispose?.());
            pieceGeometry?.dispose();
            pieceFaceGeometry?.dispose();
            stationOutlineGeometry?.dispose();
            campOutlineGeometry?.dispose();
            hqOutlineGeometry?.dispose();
