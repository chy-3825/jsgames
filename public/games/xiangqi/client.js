...
const WIDTH = 9;
const HEIGHT = 10;
const LABELS = { k: '將', a: '士', e: '象', h: '馬', r: '車', c: '砲', s: '卒' };
const RED_LABELS = { k: '帥', a: '仕', e: '相', h: '馬', r: '車', c: '炮', s: '兵' };
const PIECE_FONT = '"HanWang LiSu", "Noto Serif CJK SC", SimSun, serif';
const RIVER_FONT = '"Xiangqi River", "HanWang LiSu", "AR PL UKai CN", "AR PL UKai TW", "KaiTi", "STKaiti", "Kaiti SC", "Noto Serif CJK SC", serif';
const WOOD_TONES = ['#f1d18c', '#e9c57e', '#f3d99b', '#e5bd72'];
const RIVER_INSCRIPTIONS = [
    // SVG 的 y 轴向下，和 Three.js 的平面旋转方向相反，所以 2D 角度取反。
    { text: '楚河', worldX: -1.8, rotationY: Math.PI / 2, svgX: 245, svgY: 500, svgRotation: -90 },
    { text: '漢界', worldX: 1.8, rotationY: -Math.PI / 2, svgX: 655, svgY: 500, svgRotation: 90 }
];

export function createGameClient({ mount, send, addLog, leaveRoom }) {
    const style = document.createElement('link'); style.rel = 'stylesheet'; style.href = `/games/xiangqi/style.css?v=${Date.now()}`; document.head.appendChild(style);
    mount.innerHTML = `<section class="xiangqi3d-app"><header class="xiangqi3d-header"><button data-ui="leave" type="button">← <span>回到大厅</span></button><div class="xiangqi3d-title"><b>將</b><div><strong>中国象棋</strong><small>RIVER · PALACE · CANNON</small></div></div><div class="xiangqi3d-actions"><span data-role="room">XIANGQI TABLE</span><button data-ui="viewMode" data-role="viewModeButton" type="button">2D</button><button data-ui="reset" data-role="resetButton" type="button">视角</button><button data-ui="rules" type="button">规则</button></div></header><main class="xiangqi3d-main"><aside class="xiangqi3d-player" data-role="black"></aside><section class="xiangqi3d-stage"><div class="xiangqi3d-viewport" data-role="viewport"><canvas data-role="canvas" aria-label="三维中国象棋棋盘"></canvas><div class="xiangqi2d-board" data-role="board2d" aria-label="二维中国象棋棋盘" hidden></div><div class="xiangqi3d-status" data-role="status"></div><div class="xiangqi3d-hint" data-role="hint">选择自己的棋子</div></div></section><aside class="xiangqi3d-player" data-role="red"></aside></main><footer class="xiangqi3d-footer"><div data-role="turn"></div><div data-role="last"></div><div class="xiangqi3d-log" data-role="log"></div></footer><div class="xiangqi3d-overlay is-hidden" data-role="rulesOverlay"><article><button data-ui="closeRules" type="button">×</button><small>HOW TO PLAY</small><h2>中国象棋</h2><p>红黑双方轮流移动棋子，目标是将死或困毙对方将帅。</p><ul><li>车走直线，炮隔一子吃子，马走日字但会被马腿阻挡。</li><li>士和将不能离开九宫，象不能过河，兵过河后可横走。</li><li>将帅不能照面；被将死或没有任何合法着法的一方判负。</li><li>同一方连续第 2 次将军会收到长将警告；继续形成三次重复局面时，长将方判负。</li></ul></article></div></section>`;
    ...
    try { renderer = new THREE.WebGLRenderer({ canvas, antialias: true, powerPreference: 'high-performance' }); } catch (error) { viewport.innerHTML = '<div class="xiangqi3d-render-error"><strong>3D 棋盘无法启动</strong><span>当前浏览器没有可用的 WebGL 图形加速。</span></div>'; addLog?.(`中国象棋 3D 初始化失败：${error.message}`, 'error'); return { gameType: 'xiangqi', handleMessage() {}, destroy() { style.remove(); mount.innerHTML = ''; } }; }
    ...
    let state = null; let previous = new Map(); let selected = null; let perspectiveColor = null; let viewMode = localStorage.getItem('jsgames.xiangqi.viewMode') === '2d' ? '2d' : '3d'; let yaw = 0; let pitch = .75; let distance = 17; let dragging = false; let dragId = null; let lastPointer = { x: 0, y: 0 }; let frame = null; let renderUntil = 0; let destroyed = false; let graphicsFailed = false; const resizeTimers = [];
    ...
    function playerCard(el, player) { if (!player) { el.innerHTML = ''; return; } const color = player.color; el.className = `xiangqi3d-player ${color} ${player.isCurrentTurn ? 'is-current' : ''}`; el.innerHTML = `<small>${color === 'red' ? 'RED · 帥' : 'BLACK · 將'}</small><b>${escapeHtml(player.name.slice(0, 1))}</b><strong>${escapeHtml(player.name)}${player.id === state.myId ? ' · 我' : ''}</strong><span>${player.isCurrentTurn ? '正在行动' : player.isOnline === false ? '已离线' : color === state.myColor ? '你的棋子' : '等待中'}</span>`; }
    ...
    function setViewMode(mode) { viewMode = mode === '2d' ? '2d' : '3d'; if (viewMode === '2d') settleAnimations(); localStorage.setItem('jsgames.xiangqi.viewMode', viewMode); canvas.hidden = viewMode === '2d'; board2d.hidden = viewMode !== '2d'; viewport.classList.toggle('is-2d', viewMode === '2d'); viewModeButton.textContent = viewMode === '3d' ? '2D' : '3D'; viewModeButton.title = `切换到 ${viewModeButton.textContent} 棋盘`; resetButton.hidden = viewMode === '2d'; if (viewMode === '2d') renderBoard2d(); else { resize(); requestRender(360); } }
    function onUi(event) { const boardSquare = event.target.closest('[data-board-square]'); if (boardSquare) { activateSquare({ x: Number(boardSquare.dataset.x), y: Number(boardSquare.dataset.y) }); return; } const ui = event.target.closest('[data-ui]')?.dataset.ui; if (ui === 'leave') leaveRoom?.(); if (ui === 'viewMode') setViewMode(viewMode === '3d' ? '2d' : '3d'); if (ui === 'reset') { yaw = 0; pitch = .75; distance = 17; updateCamera(); } if (ui === 'rules') overlay.classList.remove('is-hidden'); if (ui === 'closeRules' || event.target === overlay) overlay.classList.add('is-hidden'); }
    ...
    environment(); updateCamera(); scheduleResize(); const observer = typeof ResizeObserver === 'function' ? new ResizeObserver(resize) : null; observer?.observe(viewport); viewport.addEventListener('pointerdown', onPointerDown); viewport.addEventListener('pointermove', onPointerMove); viewport.addEventListener('pointerup', onPointerUp); viewport.addEventListener('pointercancel', onPointerUp); viewport.addEventListener('wheel', event => { if (viewMode === '2d') return; event.preventDefault(); distance = THREE.MathUtils.clamp(distance + event.deltaY * .008, 14, 22); updateCamera(); }, { passive: false }); viewport.addEventListener('contextmenu', event => event.preventDefault()); mount.addEventListener('click', onUi); canvas.addEventListener('webglcontextlost', event => { event.preventDefault(); addLog?.('中国象棋 WebGL 上下文已丢失', 'error'); }); setViewMode(viewMode); Promise.all([document.fonts?.load?.('520px "HanWang LiSu"'), document.fonts?.load?.('340px "Xiangqi River"')]).then(() => { if (destroyed) return; faceMaterials.forEach(entry => entry.dispose()); faceMaterials.clear(); for (const [key, map] of textures) if (key.startsWith('piece-face-') || key.startsWith('river-ink|')) { map.dispose(); textures.delete(key); } riverSvgImages.clear(); riverTextMeshes.forEach(mesh => { mesh.material.map = verticalRiverTexture(mesh.userData.riverText); mesh.material.needsUpdate = true; }); roots.forEach(root => board.remove(root)); roots.clear(); previous.clear(); if (state) { syncPieces(state.pieces || []); renderMarkers(); renderBoard2d(); requestRender(200); } });
    return { gameType: 'xiangqi', handleMessage(message) { if (message.state) { state = message.state; if (selected && !pieceAt(selected.x, selected.y)) selected = null; renderState(); } if (message.type === 'error') addLog(message.message || '操作失败', 'error'); else if (message.action?.message) addLog(message.action.message, 'info'); }, destroy() { destroyed = true; if (frame !== null) cancelAnimationFrame(frame); resizeTimers.splice(0).forEach(timer => clearTimeout(timer)); observer?.disconnect(); faceMaterials.forEach(entry => entry.dispose()); textures.forEach(map => map.dispose()); surfaceTexture.dispose(); pieceBodyGeometry.dispose(); pieceRingGeometry.dispose(); pieceInnerRingGeometry.dispose(); pieceFaceGeometry.dispose(); pieceShadowGeometry.dispose(); checkGlowGeometry.dispose(); checkGlowMaterial.dispose(); pieceBodyMaterials.forEach(entry => entry.dispose()); Object.values(markerGeometry).forEach(entry => entry.dispose?.()); Object.values(markerMaterial).forEach(entry => entry.dispose?.()); Object.values(material).forEach(entry => entry.dispose?.()); renderer.forceContextLoss?.(); renderer.dispose(); style.remove(); mount.innerHTML = ''; } };
}
