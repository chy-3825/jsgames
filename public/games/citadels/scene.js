...
export function createCitadelsScene({ mount, model, getElement, windowRef = globalThis.window || globalThis }) {
    ...
    async function playAssassination(event, token) { const meta = roleMeta(event.role); await routePresentation(event, token, { kind: 'assassination-resolved', kicker: '密令在召集时生效', title: `${meta.name}遇刺`, copy: `${event.playerName}本轮缺席，但并未永久退出游戏`, visual: eventRoleMarkup(event.role, 'is-assassinated'), from: roleAnchor(event.role?.id), to: playerAnchor(event.playerId), color: '#cf665f', lineClass: 'is-danger', strong: true, centerDuration: 560, endDuration: 850 }); }
    ...
    async function playFinalSettlement(event, token) { const winnerIds = new Set((event.winners || []).map(item => String(item.id))); const rows = (event.standings || []).map((score, index) => `<article class="${winnerIds.has(String(score.id)) ? 'is-winner' : ''}"><b>${String(index + 1).padStart(2, '0')}</b><span><strong>${escapeHtml(score.name)}</strong><small>城区 ${score.districtSum} + 首建 ${score.firstFinisherBonus || 0} + 八城 ${score.eightCityBonus || 0} + 五色 ${score.colorBonus || 0} + 宝库 ${score.treasuryBonus || 0} + 地图室 ${score.mapRoomBonus || 0}</small></span><em>${score.score}</em></article>`).join(''); const names = (event.winners || []).map(item => item.name).join('、'); showPresentation('final-settlement', `<article class="citadels-finale-event"><span class="citadels-event-kicker">八座城市的最终账簿</span><span class="citadels-finale-crown">♛</span><h2>${escapeHtml(names || '本局')}获胜</h2><div class="citadels-finale-list">${rows}</div><p>总分相同时依次比较城区分与剩余金币</p></article>`, { strong: true }); await nextFrame(); $('presentationLayer').classList.add('is-revealed'); await presentationDelay(2800, token); }
    ...
    function enqueuePresentation(batch) { model.presentationQueue.push(JSON.parse(JSON.stringify(batch))); void runPresentationQueue(); }
    function stopPresentation() { model.presentationToken += 1; model.presentationQueue = []; for (const waiter of model.presentationWaiters) { windowRef.clearTimeout(waiter.timer); waiter.resolve(false); } model.presentationWaiters.clear(); hidePresentation(); model.presentationPlaying = false; mount.querySelector('.citadels-app')?.classList.remove('is-presentation-playing'); }
