...
export function createDecryptoActions({ mount, model, renderer, scene, rulesModal, send, documentRef = globalThis.document, windowRef = globalThis.window || globalThis }) {
    ...
    function handleInput(event) { if (scene.isPlaying()) return; const form = event.target.closest('[data-text-form]'); if (form) updateTextForm(form); }
    function handleSubmit(event) { if (scene.isPlaying()) { event.preventDefault(); return; } const form = event.target.closest('[data-text-form]'); if (!form) return; event.preventDefault(); const values = [...form.querySelectorAll('[data-text-index]')].map(input => input.value.trim()); if (!values.every(Boolean) || new Set(values.map(value => value.toLocaleLowerCase())).size !== values.length) return; if (form.dataset.textForm === 'clue') send({ type: 'gameAction', action: { kind: 'submitClue', clues: values } }); if (form.dataset.textForm === 'tiebreak') send({ type: 'gameAction', action: { kind: 'tiebreakGuess', keywords: values } }); }
    function handlePointerDown(event) { if (scene.isPlaying()) return; const hold = event.target.closest('[data-code-hold]'); if (!hold) return; event.preventDefault(); model.codePointerId = event.pointerId; hold.setPointerCapture?.(event.pointerId); renderer.setCodeVisible(true); if (state()?.phase === 'clue') { renderer.renderKeywords(); renderer.renderCommand(); } }
    ...
    function handleKeydown(event) { if (scene.isPlaying()) { if (event.key === 'Escape') scene.skipScene(); return; } if (trapOverlayFocus(event)) return; if (event.key === 'Escape' && model.activeOverlay) { closeOverlay(model.activeOverlay); return; } const hold = event.target.closest?.('[data-code-hold]'); if (!hold || (event.key !== ' ' && event.key !== 'Enter') || event.repeat) return; event.preventDefault(); model.codeRevealKey = event.key; renderer.setCodeVisible(true); if (state()?.phase === 'clue') { renderer.renderKeywords(); renderer.renderCommand(); } }
