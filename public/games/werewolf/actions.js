...
export function createWerewolfActions({ mount, model, renderer, scene, rulesModal, send, documentRef = globalThis.document, windowRef = globalThis.window || globalThis }) {
    ...
    function sendAction(action) {
        if (scene?.isPlaying?.() && !['switchSeat', 'presentationComplete', 'timerTick'].includes(action?.kind)) return false;
    ...
    function revealRole() {
        renderer.setRoleIdentityVisible(true);
        if (state()?.phase === 'roleReveal') renderer.renderAction();
    }
    ...
    function openTargetDialog(type) {
        ...
        const current = state();
        const definitions = {
            guard: { title: '第一步 · 选择守护目标', description: '选择今晚要保护的玩家，或选择空守；下一步还可以再次核对。', action: { kind: 'stageNightAction' }, targets: current.legalTargetSeats || [] },
            wolf: { title: `第 ${current.wolfVote?.round || 1} 轮袭击投票`, description: '选择一名袭击目标；提交后立即记为临时票，倒计时结束前可随时修改。', action: { kind: 'nightAction' }, targets: current.legalTargetSeats || [] },
            ...
            'wolf-self-destruct': { title: '确认自爆？', description: '确认后立即出局并结束当前白天，此操作不可撤销。', action: { kind: 'wolfSelfDestruct' }, targets: [], noTarget: true },
    ...
    function renderTargetDialog() {
        ...
        $('targetSelection').textContent = dialog.action.kind === 'wolfSelfDestruct' ? '这项操作不可撤销' : dialog.noTarget ? '这次不选择任何目标' : dialog.selected != null ? `你选择了 ${dialog.selected} 号玩家` : '还没有选择';
        const confirm = mount.querySelector('[data-ui="confirmTarget"]'); confirm.disabled = !dialog.noTarget && dialog.selected == null; confirm.textContent = dialog.action.kind === 'nightAction' ? '提交临时票' : dialog.action.kind === 'stageNightAction' ? '下一步：确认' : ['vote', 'sheriffVote'].includes(dialog.action.kind) ? '确定投票' : dialog.action.kind === 'sheriffBadgeAction' ? '确定移交' : '确定选择';
    }
    ...
    function handleClick(event) {
        ...
        if (ui === 'voice') { model.voiceEnabled = !model.voiceEnabled; if (!model.voiceEnabled) scene.cancelSpeech?.(); renderer.updateVoiceButton(); return; }
        ...
        if (ui === 'confirmTarget' && model.targetDialog) { const action = { ...model.targetDialog.action }; if (model.targetDialog.selected != null && !model.targetDialog.noTarget) action.targetSeat = model.targetDialog.selected; closeTargetDialog(); return sendAction(action); }
        ...
        if (event.target.closest('[data-action="confirmRole"]')) { if (!model.hasViewedRole) return; return sendAction({ kind: 'confirmRole' }); }
        ...
        if (event.target.closest('[data-action="startRunoffSpeech"]')) return sendAction({ kind: 'startRunoffSpeech' });
        if (event.target.closest('[data-action="finishRunoffSpeech"]')) return sendAction({ kind: 'finishRunoffSpeech' });
    ...
    function handlePointerdown(event) {
        ...
        event.preventDefault(); model.roleRevealPointerId = event.pointerId; model.roleRevealKey = null; revealRole();
    }
    ...
    function handleKeydown(event) {
        ...
        event.preventDefault(); if (!event.repeat) { model.roleRevealPointerId = null; model.roleRevealKey = event.key; revealRole(); }
    }
