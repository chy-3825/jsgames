...
export function createWerewolfRenderer({ mount, model, getElement, send, windowRef = globalThis.window || globalThis }) {
    ...
    function scheduleTimedFlow() {
        ...
        const flow = current?.phase === 'lastWords' ? current.lastWordsFlow : current?.phase === 'day' ? current.speechFlow : null;
        const timed = sheriffPhase ? current?.sheriff : flow;
        ...
        model.timedFlowInterval = windowRef.setInterval(() => {
            ...
            const currentSeat = model.state?.phase === 'day' ? model.state.canFinishSpeech : model.state?.phase === 'lastWords' ? model.state.canFinishLastWords : Boolean(model.state?.sheriffAction);
    ...
    function renderTimedTurnAction(kind, activeSeat) {
        const current = state(); const flow = kind === 'lastWords' ? current.lastWordsFlow : current.speechFlow;
        const isCurrent = Number(flow?.currentSeat) === Number(current.activeSeat);
        const startAction = kind === 'lastWords' ? 'startLastWords' : 'startSpeech'; const finishAction = kind === 'lastWords' ? 'finishLastWords' : 'finishSpeech';
        const canStart = kind === 'lastWords' ? current.canStartLastWords : current.canStartSpeech; const canFinish = kind === 'lastWords' ? current.canFinishLastWords : current.canFinishSpeech; const title = kind === 'lastWords' ? '遗言' : '依序发言';
        if (!activeSeat?.alive && kind === 'day') return '<div class="ww-waiting">你已经出局，请听存活玩家依次发言。</div>';
        if (!isCurrent) return `<div class="ww-turn-panel"><strong>${escapeHtml(flow?.currentSeat ? `${flow.currentSeat} 号玩家正在${title}` : `${title}即将开始`)}</strong><span>现在请听这位玩家发言，轮到你时界面会亮起。</span></div>`;
        ...
        if (canFinish || flow?.status === 'speaking') { const remaining = flowRemainingSeconds(flow); const input = kind === 'lastWords' ? '<textarea data-role="lastWordsText" maxlength="240" placeholder="可选：在这里写下遗言，也可以直接说出来"></textarea>' : ''; return `<div class="ww-turn-panel is-current"><strong>你正在${title}</strong><b class="ww-turn-timer" data-role="timedRemaining">${remaining}s</b>${input}<button class="ww-confirm-button is-ready" data-action="${finishAction}" type="button"><span>✓</span><b>结束${title}</b><small>时间用尽也会自然结束</small></button></div>`; }
    ...
    function renderAction() {
        ...
        const phase = current.phase || 'waiting'; let html = `<div class="ww-action-head"><div><small>游戏引导</small><h3>${escapeHtml(current.phaseName || '静候开局')}</h3></div><span class="ww-auto-badge">${current.flowPaused ? '等待玩家归来' : current.testMode ? '测试模式 · 自动继续' : '行动完成后继续'}</span></div>`;
        ...
        if (phase === 'roleReveal') html += current.myRoleConfirmed ? `<div class="ww-skill-complete">你已记下身份，还差 ${Math.max(0, current.phaseProgress.total - current.phaseProgress.completed)} 位玩家。请不要向身边的人展示。</div>` : `<p>请仔细阅读上方身份与技能，记住后遮住卡面再继续。</p><button class="ww-confirm-button is-ready" data-action="confirmRole" type="button"><span>✓</span><b>我已记住身份</b><small>${current.phaseProgress.total || current.playerCount} 人全部确认后，第一夜降临</small></button>`;
        ...
        else if (current.canConfirmDeadRole) html += '<p>你已经出局，今夜无需发动这个身份的技能。确认后请继续保持安静。</p><button class="ww-confirm-button" data-action="confirmDeadRole" type="button"><span>过</span><b>今夜不行动</b><small>你的身份仍然不会公开</small></button>';
        ...
        else if (phase === 'nightWolf' && role === 'werewolf') html += current.wolfVote?.myTarget ? `<div class="ww-skill-complete">你选择了 ${current.wolfVote.myTarget} 号，其他狼人仍在决定（${current.wolfVote.submittedCount}/${current.wolfVote.totalWolves}）。</div>` : `<p>${current.wolfVote?.round === 2 ? `第一轮平票${current.wolfVote.tiedTargets?.length ? `（${current.wolfVote.tiedTargets.join('、')}号）` : ''}，请进行最后一轮投票。` : '每名存活狼人各投一票，所有人选择后再一同揭晓。'}</p><button class="ww-skill-button is-ready" data-open-skill="wolf" type="button"><span>票</span><b>选择袭击目标</b><small>第 ${current.wolfVote?.round || 1} 轮 · 不能选择狼人同伴</small></button>`;
        ...
        else if (phase === 'nightWitch' && role === 'witch') html += `<p>女巫请睁眼。今夜遇袭的是：${current.wolfSeat ? `${current.wolfSeat} 号` : '无人'}。解药 ${current.witchItems?.antidote ? '尚在' : '已用'}，毒药 ${current.witchItems?.poison ? '尚在' : '已用'}。</p><div class="ww-witch-skills">${current.witchItems?.antidote && current.wolfSeat ? '<button class="ww-skill-button is-ready" data-stage-night-choice="save" type="button"><span>救</span><b>使用解药</b><small>救下今夜遇袭的玩家</small></button>' : ''}${current.witchItems?.poison ? '<button class="ww-skill-button is-ready" data-open-skill="witch-poison" type="button"><span>毒</span><b>使用毒药</b><small>选择一名毒药目标</small></button>' : ''}<button class="ww-skill-button" data-stage-night-choice="pass" type="button"><span>过</span><b>不使用药物</b><small>今夜收起两只药瓶</small></button></div>`;
        ...
        else if (phase === 'vote') html += !activeSeat?.alive ? '<div class="ww-waiting">你已经出局，本轮没有放逐票。</div>' : `<p>第 ${current.dayVoteRound || 1} 轮：你的放逐票${!current.canVote ? current.myVote == null ? '已经弃权' : `已经投给 ${current.myVote} 号` : '还没有投出'}。目前已有 ${current.voteProgress.completed}/${current.voteProgress.total} 人投票。</p>${current.dayVoteRound === 2 ? `<p>本轮可投给 ${current.dayTieTargets?.join('、')} 号，也可弃权。</p>` : ''}${current.canVote ? '<div class="ww-witch-skills"><button class="ww-vote-button" data-open-vote type="button">选择放逐目标</button><button class="ww-skill-button" data-action="voteAbstain" type="button"><span>弃</span><b>弃权</b><small>本轮不投给任何人</small></button></div>' : '<div class="ww-skill-complete">你的放逐票已经提交，请等待其他玩家。</div>'}`;
    ...
    function render() {
        ...
        appRoot?.classList.toggle('is-night', isNight); appRoot?.classList.toggle('is-day', !isNight && current.status !== 'ended'); appRoot?.classList.toggle('is-my-turn', Boolean(isNight && current.skillState?.available)); appRoot?.classList.toggle('is-flow-paused', Boolean(current.flowPaused)); appRoot?.classList.toggle('is-test-mode', Boolean(current.testMode)); appRoot?.classList.toggle('is-my-action', Boolean(current.skillState?.available || current.nightConfirmation || current.canVote || current.canConfirmDeathResolution || current.canStartSpeech || current.canStartLastWords || current.sheriffAction?.available));
        ...
        const screenSeat = mount.querySelector('[data-role="screen-seat"]'); if (screenSeat) screenSeat.textContent = current.activeSeat ? current.testMode ? `正在查看 ${current.activeSeat} 号` : `你是 ${current.activeSeat} 号玩家` : '等待入座';
        ...
        $('seats').innerHTML = seats.map(seat => { const role = current.testMode ? seat.role || model.testRoleBySeat.get(seat.number) : ''; const roleName = role ? ROLE[role]?.name || role : ''; const seatLabel = roleName || seat.controllerName || (current.testMode ? '虚拟座位' : '等待入座'); const statusLabel = roleName ? `${seat.controllerName || '虚拟座位'} · ${seatStatus(seat)}` : seatStatus(seat); return `<button type="button" data-seat="${seat.number}" class="ww-seat ${seat.number === current.activeSeat ? 'is-active' : ''} ${seat.alive ? '' : 'is-dead'} ${current.phase === 'roleReveal' && seat.roleConfirmed ? 'is-confirmed' : ''} ${current.phase === 'day' && seat.alive ? seat.dayReady ? 'is-day-ready' : 'is-day-pending' : ''}" ${seat.canControl ? '' : 'disabled'}><b>${seat.number}</b><span>${escapeHtml(seatLabel)}</span><small>${escapeHtml(statusLabel)}</small></button>`; }).join('') || '<p class="ww-empty">夜幕尚未降临…</p>';
        ...
        renderSheriffState(); renderAnnouncement(); renderVoteResult(); renderRole(); renderAction(); scheduleTimedFlow();
