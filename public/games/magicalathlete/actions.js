...
export function createMagicalAthleteActions({ mount, model, renderer, scene, send, addLog, documentRef = globalThis.document, windowRef = globalThis.window || globalThis }) {
    ...
    function submitAction(action) { if (!action || model.actionPending || model.presentationPlaying) return false; model.actionPending = true; clearError(); renderer.render(); send({ type: 'gameAction', action }); return true; }
    function chooseAction(payload, title, detail, key) { model.pendingAction = { payload, title, detail, key }; renderer.renderCommand(); }
    function setRulesOpen(open) { const overlay = $('rulesOverlay'); overlay.classList.toggle('is-hidden', !open); overlay.setAttribute('aria-hidden', String(!open)); if (open) { model.previousFocus = documentRef.activeElement; [...mount.querySelector('.ma-app').children].forEach(child => { child.inert = child !== overlay; }); windowRef.requestAnimationFrame?.(() => overlay.querySelector('[data-ui="closeRules"]')?.focus()); } else { [...mount.querySelector('.ma-app').children].forEach(child => { child.inert = false; }); model.previousFocus?.focus?.(); model.previousFocus = null; } }
    ...
    function handleClick(event) {
        ...
        if (ui === 'acknowledgeElimination') { const acknowledgement = state()?.acknowledgement; if (!acknowledgement || acknowledgement.playerId !== state().myId || model.actionPending || model.presentationPlaying) return; submitAction({ kind: 'acknowledgeElimination', acknowledgementId: acknowledgement.id }); return; }
    ...
    function handleKeydown(event) { if (scene.isPlaying()) { if (event.key === 'Escape') scene.skipPresentation(); event.preventDefault(); return; } if (trapRulesFocus(event)) return; if (event.key !== 'Escape') return; if (!$('rulesOverlay').classList.contains('is-hidden')) setRulesOpen(false); else if (model.pendingAction) { model.pendingAction = null; model.actionPending = false; renderer.renderCommand(); } }
    return { handleClick, handleKeydown, submitAction, chooseAction, setRulesOpen, trapRulesFocus };
}
