...
const GLYPHS = { white: { p: '♙', n: '♘', b: '♗', r: '♖', q: '♕', k: '♔' }, black: { p: '♟', n: '♞', b: '♝', r: '♜', q: '♛', k: '♚' } };
const STARTING_TYPES = ['r', 'n', 'b', 'q', 'k', 'b', 'n', 'r'];
const SKIN_STORAGE_KEY = 'jsgames.chess.skin';
const CHESS_SKINS = {
    walnut: {
        label: '经典胡桃木', subtitle: '经典棋室 · 三维棋盘', scene: '#1a100c',
        ivory: '#f1e3c7', ebony: '#211813', lightTile: '#dfc79d', darkTile: '#704931', frame: '#422719', table: '#2a1710',
        ivoryDetail: '#6d5038', ebonyDetail: '#9a8060',
        hemisphereSky: '#f4e4c6', hemisphereGround: '#25140d', key: '#fff4d9', rim: '#c9915d', rimIntensity: 1.45,
    },
    slate: {
        label: '暮色石板', subtitle: '暮色棋室 · 三维棋盘', scene: '#09141d',
        ivory: '#eee5d0', ebony: '#171d26', lightTile: '#d9d4c3', darkTile: '#3d4950', frame: '#626b70', table: '#111c22',
        ivoryDetail: '#66645f', ebonyDetail: '#87909a',
        hemisphereSky: '#dce8ed', hemisphereGround: '#14212a', key: '#fff4d9', rim: '#8db9d4', rimIntensity: 1.6,
    },
};
...
export function createGameClient({ mount, send, addLog, leaveRoom }) {
    ...
    style.href = `/games/chess/chess3d.css?v=${Date.now()}`;
    document.head.appendChild(style);
    document.body.classList.add('is-chess-view');
    ...
    mount.innerHTML = `
        <section class="chess3d-app" data-skin="${skinId}">
            <header class="chess3d-header">
                <div class="chess3d-header-left"><button class="chess3d-back" data-ui="leave" type="button">← <span>回到大厅</span></button><button class="chess3d-settings-button" data-ui="settings" type="button" aria-haspopup="dialog"><span aria-hidden="true">⚙</span> 设置</button></div>
    ...
    try {
        ...
        return { gameType: 'chess', handleMessage() {}, destroy() { document.body.classList.remove('is-chess-view'); style.remove(); mount.innerHTML = ''; } };
    }
    ...
    let viewMode = localStorage.getItem('jsgames.chess.viewMode') === '2d' ? '2d' : '3d';
    let cameraYaw = 0;
    let cameraPitch = .75;
    ...
    function activateSquare(square) { if (!state || state.status === 'ended' || animations.length || pendingPromotion) return; const piece = pieceAt(square.x, square.y); if (selected && isLegalTarget(square.x, square.y)) { const moving = pieceAt(selected.x, selected.y); const promotionRank = square.y === 0 || square.y === 7; if (moving?.type === 'p' && promotionRank) { pendingPromotion = { from: selected, to: { x: square.x, y: square.y }, color: moving.color }; selected = null; showPromotionChoices(moving.color); renderTiles(); renderBoard2d(); return; } sendMove(selected, { x: square.x, y: square.y }); selected = null; renderTiles(); renderBoard2d(); return; } selected = piece && piece.color === state.myColor && state.myIsCurrentTurn && (state.legalMoves?.[piece.id] || []).length ? { x: square.x, y: square.y } : null; renderTiles(); renderBoard2d(); }
    ...
    return { gameType: 'chess', handleMessage, destroy() { destroyed = true; if (frameId !== null) cancelAnimationFrame(frameId); window.removeEventListener('resize', resize); window.removeEventListener('keydown', onKeyDown); resizeObserver?.disconnect(); effects.forEach(effect => effect.material.dispose()); effects.length = 0; activeThreatGlows.length = 0; landingEffectGeometry.dispose(); threatGlowGeometry.dispose(); threatGlowTexture.dispose(); Object.values(threatGlowMaterials).forEach(material => material.dispose()); pieceGeometries.forEach(geometry => geometry.dispose()); tileMeshes.forEach(tile => tile.material.dispose()); Object.values(materials).forEach(material => material.dispose()); renderer.forceContextLoss?.(); renderer.dispose(); document.body.classList.remove('is-chess-view'); style.remove(); mount.innerHTML = ''; } };
}
