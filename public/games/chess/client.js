...
export function createGameClient({ mount, send, addLog }) {
    ...
    style.href = '/games/chess/chess3d.css?v=20260826-mobile-games-2';
    ...
    return { gameType: 'chess', handleMessage, setStudyPlacement(mode) { studyPlacement = mode && (mode.remove || mode.type) ? { ...mode } : null; if (mode && state?.studyMode && state.studyPhase === 'setup' && viewMode !== '2d') setViewMode('2d', false); }, destroy() { destroyed = true; studyPlacement = null; if (frameId !== null) cancelAnimationFrame(frameId); window.removeEventListener('resize', resize); window.removeEventListener('keydown', onKeyDown); resizeObserver?.disconnect(); effects.forEach(effect => effect.material.dispose()); effects.length = 0; activeThreatGlows.length = 0; landingEffectGeometry.dispose(); threatGlowGeometry.dispose(); threatGlowTexture.dispose(); Object.values(threatGlowMaterials).forEach(material => material.dispose()); pieceGeometries.forEach(geometry => geometry.dispose()); tileMeshes.forEach(tile => tile.material.dispose()); Object.values(materials).forEach(material => material.dispose()); renderer.forceContextLoss?.(); renderer.dispose(); document.body.classList.remove('is-chess-view'); style.remove(); mount.innerHTML = ''; } };
}
