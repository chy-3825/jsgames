...
export function createCamelUpScene({ mount, model, getElement, windowRef = globalThis.window || globalThis, renderer, onPresentationStart = () => {} }) {
    ...
    async function animateFloating(fromElement, toElement, className, html, token, duration = 620) { const from = centerOf(fromElement); const to = centerOf(toElement); if (!from || !to) return presentationWait(220, token); const ghost = mount.ownerDocument.createElement('div'); ghost.className = className; ghost.style.left = `${from.x}px`; ghost.style.top = `${from.y}px`; ghost.innerHTML = html; $('floatingLayer').appendChild(ghost); ghost.getBoundingClientRect(); ghost.style.transform = `translate(calc(-50% + ${to.x - from.x}px), calc(-50% + ${to.y - from.y}px)) rotate(-2deg)`; const continued = await presentationWait(duration, token); ghost.remove(); return continued; }
    async function animateCamelStack(event, token) { const from = trackAnchor(event.from); const to = trackAnchor(event.to); const camelIds = (event.movingCamels || []).map(camel => camel.id); camelIds.forEach(id => findData('data-camel-id', id)?.classList.add('is-presentation-destination')); const markup = `<span class="cm-floating-camel-stack">${(event.movingCamels || []).map((camel, index) => { const meta = camelMeta(camel.id); return `<i style="--camel:${meta.color};--stack:${index}">${camelGlyph('cm-floating-camel')}<small>${meta.mark}</small></i>`; }).join('')}</span>`; const continued = await animateFloating(from, to, 'cm-floating-camels', markup, token, 720); camelIds.forEach(id => findData('data-camel-id', id)?.classList.remove('is-presentation-destination')); return continued; }
    async function playPresentationEvent(event, token) {
        ...
        if (event.kind === 'raceStarted') { const positions = [1, 2, 3].map(position => { const stack = (event.camels || []).filter(camel => Number(camel.position) === position).sort((left, right) => Number(left.order) - Number(right.order)); return `<span><b>${position} 号格</b><span class="cm-starting-stack">${stack.map(camel => eventCamel(camel)).join('') || '—'}</span></span>`; }).join(''); showPresentation('major', 'start', '沙漠大赛正式开始', '起跑骰已经揭晓', `<div class="cm-starting-stacks">${positions}</div><p class="cm-event-note">${escapeHtml(event.currentPlayerName || '首位玩家')}首先行动 · 每位玩家拥有 ${event.startingCoins || 3} 枚起始金币</p>`); await presentationWait(1200, token); return; }
        if (event.kind === 'dieRevealed') { const meta = camelMeta(event.camel?.id); showPresentation('compact', 'dice', '骰塔开启', `${event.actorName}掷出 ${meta.name}·${event.steps} 格`, `<div class="cm-event-die" style="--camel:${meta.color}"><i>${meta.mark}</i><b>${event.steps}</b><small>获得 1 块金字塔牌</small></div>`); drawActionLine(playerAnchor(event.actorId), pyramidAnchor()); await presentationWait(720, token); return; }
        if (event.kind === 'camelMoved') { const count = event.movingCamels?.length || 1; showPresentation('compact', 'move', '驼队移动', `${event.camel?.name}前进 ${event.steps} 格`, `<div class="cm-event-route">${eventCamel(event.camel, count > 1 ? `携带上方 ${count - 1} 匹` : '单独移动')}<i>${event.from}</i><b>→</b><i>${event.to}</i></div>`); drawActionLine(trackAnchor(event.from), trackAnchor(event.to)); if (!await animateCamelStack(event, token)) return; await presentationWait(260, token); return; }
        if (event.kind === 'desertTileTriggered') { showPresentation('medium', 'tile', event.tileType === 'oasis' ? '绿洲触发' : '海市蜃楼触发', `${event.camel?.name}${event.tileType === 'oasis' ? '再前进 1 格' : '后退 1 格'}`, `<div class="cm-event-route">${eventTile(event.tileType, event.tilePosition)}<i>${event.from}</i><b>→</b><i>${event.to}</i></div><p class="cm-event-note">${escapeHtml(event.ownerName || '板块主人')}${event.reward ? '获得 1 金币' : ''}</p>`); drawActionLine(trackAnchor(event.from), trackAnchor(event.to)); if (!await animateCamelStack(event, token)) return; if (event.reward && playerAnchor(event.ownerId)) { drawActionLine(trackAnchor(event.tilePosition), playerAnchor(event.ownerId), 'money'); await presentationWait(520, token); } return; }
        if (event.kind === 'legBetTaken') { showPresentation('compact', 'bet', '赛段下注', `${event.actorName}押注${event.camel.name}`, `<div class="cm-event-transaction">${eventLegCard(event.camel, event.payout)}<i>→</i>${eventPlayer(event.actorId, event.actorName, `当前牌面 ${event.payout}`)}</div><p class="cm-event-note">该颜色牌架还剩 ${event.remaining} 张</p>`); drawActionLine(playerAnchor(event.actorId), legAnchor(event.camel.id)); if (!await animateFloating(legAnchor(event.camel.id), playerAnchor(event.actorId), 'cm-floating-card', eventLegCard(event.camel, event.payout), token)) return; await presentationWait(280, token); return; }
        if (event.kind === 'overallBetPlaced') { const own = event.private; const face = own ? eventFinishCard(own.camelId, event.outcome, false) : eventFinishCard(null, event.outcome, true); showPresentation('compact', 'secret', '终局暗注', `${event.actorName}放入${event.outcome === 'loser' ? '垫底' : '冠军'}区`, `${face}<p class="cm-event-note">第 ${event.order} 张入堆·牌面保密至冲线</p>`); drawActionLine(playerAnchor(event.actorId), overallAnchor(event.outcome), 'secret'); if (!await animateFloating(playerAnchor(event.actorId), overallAnchor(event.outcome), 'cm-floating-card', eventFinishCard(null, event.outcome, true), token)) return; await presentationWait(280, token); return; }
        if (event.kind === 'desertTilePlaced') { showPresentation('compact', 'tile', event.moved ? '移动沙漠板块' : '布置沙漠板块', `${event.actorName}选择 ${event.position} 号格`, `${eventTile(event.tileType, event.position)}<p class="cm-event-note">${event.tileType === 'oasis' ? '驼队经过时前进并叠到上方' : '驼队经过时后退并垫到下方'}</p>`); drawActionLine(playerAnchor(event.actorId), trackAnchor(event.position)); await animateFloating(event.moved ? trackAnchor(event.previousPosition) : playerAnchor(event.actorId), trackAnchor(event.position), 'cm-floating-tile', eventTile(event.tileType), token); return; }
